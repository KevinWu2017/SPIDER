...
#define D_BLOCK_COL (BLOCK_COL + HALO * 2 + 8)
...
__global__ void
kernel_2d_7r(const TYPE *__restrict__ in, const TYPE *__restrict__ params, const uint *__restrict__ metadata, TYPE *__restrict__ out, const int ldm) {
    ...
    for (int block_iter_row = 0; block_iter_row < BLOCK_ITER_ROW; block_iter_row++) {
        ...
        for (int block_iter_col = 0; block_iter_col < BLOCK_ITER_COL; block_iter_col++) {
            ...
            #pragma unroll
            for (int iter_n = 0; iter_n < BLOCK_COL / SPTC_N; iter_n++){
                #pragma unroll
                for (int iter_param = 0; iter_param < 15; iter_param++){
                    #pragma unroll
                    for (int iter_k = 0; iter_k < 32 / SPTC_K; iter_k++) {
                        frag_params[iter_k * 2 + 0] = *((uint *)(params + IDX(lane_id / 4, iter_k * 8 + lane_id % 4 * 2, 16) + IDX(iter_param, 0, 16*16)));
                        frag_params[iter_k * 2 + 1] = *((uint *)(params + IDX(lane_id / 4 + 8, iter_k * 8 + lane_id % 4 * 2, 16) + IDX(iter_param, 0, 16*16)));

                        ((half*)frag_meta)[0] = ((half*)metadata)[IDX(lane_id / 4, iter_k, 2)];
                        ((half*)frag_meta)[1] = ((half*)metadata)[IDX(lane_id / 4 + 8, iter_k, 2)];

                        int iter_begin = warp_begin + IDX(iter_k*SPTC_K, iter_n*SPTC_N+iter_param+1, D_BLOCK_COL);

                        int swap_temp = 0;
                        if (iter_k == 0) {
                            swap_temp = IDX(16, 0, D_BLOCK_COL);
                        } else {
                            swap_temp = -IDX(16, 0, D_BLOCK_COL);
                        }

                        ((half*)frag_in)[0] = sharedmem[iter_begin + swap_temp + IDX(lane_id%4*2    , lane_id / 4, D_BLOCK_COL)];
                        ((half*)frag_in)[1] = sharedmem[iter_begin + IDX(lane_id%4*2 + 1, lane_id / 4, D_BLOCK_COL)];
                        ((half*)frag_in)[2] = sharedmem[iter_begin + swap_temp + IDX(lane_id%4*2 + 8, lane_id / 4, D_BLOCK_COL)];
                        ((half*)frag_in)[3] = sharedmem[iter_begin + IDX(lane_id%4*2 + 9, lane_id / 4, D_BLOCK_COL)];
                            ...
                            asm volatile(
                                ...
                                "r"(frag_acc[iter_n][0]), "r"(frag_acc[iter_n][1]), "r"(frag_meta[0])
                            );
                    }
...
void param_swap_to_structured_sparsity(const TYPE * params, TYPE * sparse_params){
    ...
    for (int iter_param = 0; iter_param < 15; iter_param++){
        for (int i = 0; i < 16; i++) {
            for (int j = 0; j < 16; j=j+2) {
                sparse_params[IDX(iter_param, 0, 16*32) + IDX(i, j, 32)] = params[IDX(iter_param, 0, 16*32) + IDX(i, j+16, 32)];
                sparse_params[IDX(iter_param, 0, 16*32) + IDX(i, j+16, 32)] = params[IDX(iter_param, 0, 16*32) + IDX(i, j, 32)];
            }
        }
    }
    ...
    for (int iter_param = 0; iter_param < 15; iter_param++){
        for (int i = 0; i < 16*32; i=i+4){
            int zero_count = 0;
            for (int j = 0; j < 4; j++) {
                if(__half2float(sparse_params[IDX(iter_param, 0, 16*32) + i+j]) == 0){
                    zero_count++;
                }
            }
            if (zero_count < 2){
                std::cout << "ITER PARAM: " << iter_param << std::endl;
...
void compress_params(TYPE * params, TYPE * compressed_params, uint * metadata){
    TYPE *compressed_origin_params = (TYPE *)malloc(15*16*16*sizeof(TYPE));
    ...
    for (int i = 0; i < 16; i++){
        for (int j = 0; j < 32; j=j+4){
            ...
            for (int iter_param = 0; iter_param < 15; iter_param++){
                compressed_origin_params[IDX(iter_param, 0, 16*16) + i*16+j/2] = params[IDX(iter_param, 0, 16*32) + i*32+j+non_zero_idx[0]];
                compressed_origin_params[IDX(iter_param, 0, 16*16) + i*16+j/2 + 1] = params[IDX(iter_param, 0, 16*32) + i*32+j+non_zero_idx[1]];
            }
    ...
    for (int iter_param = 0; iter_param < 15; iter_param++){
        for (int iter_k = 0; iter_k < 2; iter_k++){
            for (int lane_id = 0; lane_id < 32; lane_id++){
                compressed_params[lane_id * 8 + iter_k * 4 + 0 + IDX(iter_param, 0, 16 * 16)] 
                    = compressed_origin_params[IDX(lane_id / 4, lane_id % 4 * 2 + iter_k * 8, 16) 
                        + IDX(iter_param, 0, 16 * 16)];
                compressed_params[lane_id * 8 + iter_k * 4 + 1 + IDX(iter_param, 0, 16 * 16)] 
                    = compressed_origin_params[IDX(lane_id / 4, lane_id % 4 * 2 + 1 + iter_k * 8, 16) 
                        + IDX(iter_param, 0, 16 * 16)];
                compressed_params[lane_id * 8 + iter_k * 4 + 2 + IDX(iter_param, 0, 16 * 16)] 
                    = compressed_origin_params[IDX(lane_id / 4 + 8, lane_id % 4 * 2 + iter_k * 8, 16) 
                        + IDX(iter_param, 0, 16 * 16)];
                compressed_params[lane_id * 8 + iter_k * 4 + 3 + IDX(iter_param, 0, 16 * 16)] 
                    = compressed_origin_params[IDX(lane_id / 4 + 8, lane_id % 4 * 2 + 1 + iter_k * 8, 16) 
                        + IDX(iter_param, 0, 16 * 16)];
            }
        }
    }
...
void gpu_2d_7r(const TYPE *__restrict__ in, TYPE *__restrict__ out, TYPE *__restrict__ params, const int input_m, const int input_n, int times, const bool check){
    ...
    memcpy(sparse_params, params, 15*16*32*sizeof(TYPE));
    ...
    uint *metadata_d;
    CUDA_CHECK(cudaMalloc(&compressed_params_d, 15*16*16*sizeof(TYPE)));
    CUDA_CHECK(cudaMalloc(&metadata_d, 16*sizeof(uint)));
    CUDA_CHECK(cudaMemcpy(compressed_params_d, compressed_params, 15*16*16*sizeof(TYPE), cudaMemcpyHostToDevice));
    ...
    if (check) {
        times = 0;
        kernel_2d_7r<<<grid_config, block_config>>>(array_d[0], compressed_params_d, metadata_d, array_d[1], cols);
        ...
        int i = 0;
        for (; i < times; i++) {
            kernel_2d_7r<<<grid_config, block_config>>>(array_d[i % 2], compressed_params_d, metadata_d, array_d[(i + 1) % 2], cols);
            cudaDeviceSynchronize();
        }
