...
void check_result(TYPE *params, TYPE *in, TYPE *out, int m, int n) {
    ...
    for (int i = 0; i < 15; i++) {
        for (int j = 0; j < 15; j++) {
            std::cout << __half2float(cpu_params[IDX(i, j, 15)]) << " ";
        }
        std::cout << std::endl;
    }
    ...
    for (int i = 870; i < m && i < 880+26; i++) {
        for (int j = 630; j < n && j < 640+26; j++) {
            std::cout << __half2float(out[IDX(i+8, j+8, n+8*2)]) << " ";
        }
        std::cout << std::endl;
    }
    ...
    for (int i = 880; i < m && i < 880+16; i++) {
        for (int j = 640; j < n && j < 640+16; j++) {
            std::cout << __half2float(cpu_out[IDX(i, j, n)]) << " ";
        }
        std::cout << std::endl;
    }
...
int main(int argc, const char **argv) {
    ...
    std::cout << "SPTC_half_dense, " << "2d" << ", 1, " << n << ", " << m << ", " << times << ", ";
